function Test-FileExist {
  ...
  if (Test-Path -Path $FilePath) {
    ...
    if ([IO.Path]::GetExtension($FilePath) -eq $FileExtension) {
      $result | Add-Member -NotePropertyName "Log" -NotePropertyValue "Success: File path exist, file extension is $($FileExtension)"
    ...
    $result | Add-Member -NotePropertyName "Log" -NotePropertyValue "Failed: File path not exist"
    $result | Add-Member -NotePropertyName "Result" -NotePropertyValue $false
    return $result
  }
}